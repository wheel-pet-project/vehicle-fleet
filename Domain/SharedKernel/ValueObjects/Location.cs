...
public class Location : ValueObject
{
    ...
    {
        if (sizeInDegree <= 0)
            throw new ValueIsUnsupportedException($"{nameof(sizeInDegree)} must be greater than 0");
    ...
    public static Location Create(double latitude, double longitude)
    {
        if (latitude is < 0 or > 90)
            throw new ValueIsUnsupportedException($"{nameof(latitude)} must be between 0 and 90");
        if (longitude is < 0 or > 180)
            throw new ValueIsUnsupportedException($"{nameof(longitude)} must be between 0 and 180");
