...
internal sealed class ModelEntityTypeConfiguration : IEntityTypeConfiguration<Model>
{
    public void Configure(EntityTypeBuilder<Model> builder)
    {
        ...
        builder.OwnsOne(x => x.Brand, cfg =>
        {
            cfg.Property(x => x.Name).HasColumnName("brand").IsRequired();
        });

        builder.OwnsOne(x => x.CarModel, cfg =>
        {
            cfg.Property(x => x.Name).HasColumnName("car_model").IsRequired();
        });
...
internal sealed class VehicleEntityTypeConfiguration : IEntityTypeConfiguration<Vehicle>
{
    public void Configure(EntityTypeBuilder<Vehicle> builder)
    {
        ...
        builder.OwnsOne(x => x.PlateNumber, cfg =>
        {
            cfg.Property(x => x.Value).HasColumnName("plate_number").IsRequired();
        });

        builder.OwnsOne(x => x.Color, cfg =>
        {
            cfg.Property(x => x.Name).HasColumnName("color").IsRequired();
        });

        builder.OwnsOne(x => x.Vin, cfg =>
        {
            cfg.Property(x => x.Number).HasColumnName("vin").IsRequired();
        });

        builder.OwnsOne(x => x.FuelLevel, cfg =>
        {
            cfg.Property(x => x.Percents).HasColumnName("fuel_level_percents").IsRequired();
        });
